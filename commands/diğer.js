...
message.channel.send(new Discord.MessageEmbed().setColor('#000001').setDescription(`\`\`\`RedSton Diğer Kullanabileceğin Komutlar Aşağıda Yer Almakta.\`\`\`
...
**RedSton bağlantılarını gönderir**

📋 \`${client.ayarlar.prefix}kurallar\`
**RedSton hazır tema ile kurallarınızı yerleştirir**

🌟 \`${client.ayarlar.prefix}yavaş-mod\`
**RedSton üyelerinizin belirlediğiniz sürede bir mesaj yazmasını sağlar**

\`${client.ayarlar.prefix}emote\`
**Emoji URL veya Emoji girerek eğlenceli emojinin çizilimini atar**

🐝 \`${client.ayarlar.prefix}emoji\`
**RedSton sunucunda ki emoji ismini yaz sana indirme bağlantısı verir**
...
**RedSton Telefon kullananlar için Rol Renk değiştirme**

🗑 \`${client.ayarlar.prefix}sil [100]\`
**RedSton bot kanaldaki mesajları siler**

\`${client.ayarlar.prefix}sunucudan-ayrıl\`
**RedSton ile kurmuş olduğunuz ayarlarınızı sıfırlayarak sunucundan çıkar**`).setThumbnail('https://media.giphy.com/media/3qXi5eIvcgCmTDObOU/giphy.gif'));
